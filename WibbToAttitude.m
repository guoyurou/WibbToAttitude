...
%20170322 使用加速度计数据，载体不再是静止，开始处理经纬度与东向北向速度
%20170329 开始处理高度通道
clc
clear
...
for i=1:nepoch
    ...
    if mod(ResOffer.time(i),DeltaTOfH)==0
        ...
        if size(index_hc)==1
            ...
            %一阶欧拉法更新高度，K_one*(h-hc)作为vu的阻尼
            h=h+(vu-K_one*(h-hc))*DeltaTOfH*DeltaT;
            %一阶欧拉法更新天向速度，K_two*(h-hc)作为天向加速度DotVu的阻尼
            ResOffer.h(i)=h;
            vu=vu+(DotVu-K_two*(h-hc))*DeltaTOfH*DeltaT;
            ResOffer.vu(i)=vu;
            %更新DotVu供下一次高度通道解算使用
            DotVu=fn(3)+(2*wien(2)+wenn(2))*vn(1)-(2*wien(1)+wenn(1))*vn(2)-AccuG(lati,h);
            %更新矩阵vn、DotVn中对应的高度通道的结果，供程序其他部分使用
            vn(3)=vu;
            DotVn(3)=DotVu;
            %此处若size(index_hc)==1不成立，则下次一阶欧拉法数值积分时其实步长就不再是DeltaTOfH*DeltaT，应当累加，此处暂且不考虑该情况
        end
    end
end
%输出角度以度为单位    
